    ...
    public class TransportController : Controller
    {
        ...
        public IActionResult AddTransport(AddTransportViewModel viewModel, IFormFile file)
        {
            ...
            try
            {
                ...
                if (rowChanged > 0)
                {
                    return Json(new ResponseViewModel<AddTransportViewModel> { status = true, message = "Thêm thành công.", rowsNumberChanged = rowChanged, data = viewModel });
                }
                else
                {
                    return Json(new ResponseViewModel<AddTransportViewModel> { status = false, message = "Thêm không thành công.", rowsNumberChanged = rowChanged, data = null });
                }

            }
            catch (Exception ex)
            {
                return Json(new ResponseViewModel<AddTransportViewModel> { status = false, message = ex.Message, rowsNumberChanged = rowChanged, data = null });
            }
        }
    }
}
